...
import { CONTAINER, PillLink, Reveal, SplitReveal, SECTION, SectionTitle } from "@/components/sections/kit";
...
export default function Expertise() {
  ...
  useEffect(() => {
    if (reduce) return; // reduced motion: every card is rendered open, nothing to drive
        ...
        <div ref={list} className="mt-16 flex flex-col gap-3 md:mt-24">
          {services.map(([title, copy], i) => (
            ...
            <Link
              ...
              data-card
              href="/services"
              className="relative grid overflow-hidden rounded-3xl px-8 py-10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand md:px-12 md:py-14"
              style={{
                gridTemplateRows: reduce ? "auto 1fr" : "auto 0fr",
                ["--p" as string]: reduce ? 1 : 0,
                backgroundColor:
