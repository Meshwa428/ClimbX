...
- **Guitar-string dividers now repel and ring** (`components/effects/guitar-string.tsx`, hitayush's component): the control point used to tween straight onto the cursor, so the string read as stuck to the pointer and dragged around. It now uses the same `repelTarget` from `lib/strings.js` as the easter egg — pushed away, never followed — and the line is **solid `var(--color-ink)` at `strokeWidth` 2 by default** (every call site's `rgba(26,26,26,0.1x)` override deleted; a divider is a string, not a hairline rule). **Track → snap → ring → track:** it chases the cursor (0.18s, `power2.out`) until the control point is `push * 1.7` off rest, then the grip breaks — `elastic.out(1, 0.2)` over 0.9s with pointer input ignored for the first **320ms only** (locking for the whole ring left a dead line for over a second). Tuning is capped in px, not scaled off the box: `reach = min(height/2, 40)`, `push = min(height*0.28, 22)` — as fractions of a 104px box it lunged at cursors 100px away and bowed twice as hard as the Cuberto reference. Hit areas: 84 services, 104 home, 64 footer, 76 placeholder. **The lock is a timestamp, not a flag cleared from the tween's own `onUpdate`** — a tween referencing itself there throws before it's assigned and the string stays dead after one pluck (that bug shipped for one commit). Measured over CDP: no bend at 40px, apex climbs to ~17px at 22px away, snaps, re-grabbable in 0.25s, repeatable over cycles.
- **`/services` = our layout, their strings.** hitayush's `services-hero/list/process` components were deleted (the page no longer imported them); the rows-and-strings page in `app/services/page.tsx` is what ships, with a `GuitarString` between each service row instead of a border.
