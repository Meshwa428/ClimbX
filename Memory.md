...
- **Guitar-string dividers now repel and ring** (`components/effects/guitar-string.tsx`, hitayush's component): the control point used to tween straight onto the cursor, so the string read as stuck to the pointer and dragged around. It now uses the same `repelTarget` from `lib/strings.js` as the easter egg — pushed away, never followed — and the line is **solid `var(--color-ink)` at `strokeWidth` 2 by default** (every call site's `rgba(26,26,26,0.1x)` override deleted; a divider is a string, not a hairline rule). Bounce: crossing the string mid-hover *and* leaving both fire `elastic.out(1, 0.16)` over 1.6s; tracking stays a 0.18s chase, because an elastic tween restarted on every mousemove fights itself. Hit areas grew (56→84 services, 80→104 home, 50→64 footer, 60→76 placeholder) — a 50px box has no room to bow. Measured over CDP: 9 zero-crossings, ~16px peak, silent inside 1.2s.
